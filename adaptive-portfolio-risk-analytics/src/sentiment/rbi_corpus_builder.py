...
def validate_rbi_manifest(
    ...
    missing_columns = [
        column for column in REAL_RBI_MANIFEST_COLUMNS if column not in frame
    ]
    unexpected_columns = [
        column for column in frame if column not in REAL_RBI_MANIFEST_COLUMNS
    ]
    ...
    for row_number, row in frame.iterrows():
        ...
        if values["document_id"] and not DOCUMENT_ID_PATTERN.fullmatch(
            values["document_id"]
        ):
        ...
        retrieval_date = pd.to_datetime(
            values["retrieval_date"], errors="coerce"
        )
        ...
        if (
            ...
            and values["document_type"] not in REAL_RBI_DOCUMENT_TYPES
        ):
            errors.append("invalid document_type")

        resolved_path = (
            (root / values["local_path"]).resolve()
    ...
    valid = validated.loc[
        validated["validation_status"].eq("valid")
    ].copy()
    invalid = validated.loc[
        validated["validation_status"].eq("invalid")
    ].copy()
    duplicate_records = validated.loc[
        duplicate_id.to_numpy() | duplicate_identity.to_numpy()
    ].copy()
    sentences = (
        split_rbi_documents_into_sentences(valid)
        if not valid.empty
        else pd.DataFrame()
    )
    ...
    summary = {
        ...
        "missing_field_count": int(
            validated["validation_errors"].str.contains(
                "missing fields", na=False
            ).sum()
        ),
    ...
    return {
        "is_valid": bool(
            not missing_columns and not unexpected_columns and not valid.empty
        ),
        "manifest": frame,
