...
def compare_composite_nlp_to_regimes(
    ...
    *,
    ...
    if "decision_composite_nlp_label" not in composite_index:
        raise ValueError("composite_index must contain decision_composite_nlp_label")
    ...
    comparison["coverage_score"] = composite_index.get("decision_coverage_score", 0.0)
    comparison["rule_based_regime"] = rule
    comparison["hmm_walk_forward_regime"] = hmm
    comparison["agreement_rule_based"] = comparison.apply(
        lambda row: nlp_agrees_with_regime(row["composite_nlp_label"], row["rule_based_regime"]),
        axis=1,
    )
    ...
    rule_stress = rule.astype(str).str.lower().isin({"stress", "crisis", "risk-off", "risk_off"})
    hmm_stress = hmm.astype(str).str.lower().isin({"stress", "crisis", "risk-off", "risk_off"})
    ...
    source_contribution = pd.DataFrame(
        [
            {
                "source": column.removeprefix("decision_").removesuffix("_risk_score"),
                "mean_absolute_risk_contribution": float(
                    pd.to_numeric(composite_index.get(column), errors="coerce").abs().mean()
                ),
            }
    ...
    return {
        ...
        "agreement_with_rule_based": _rate(comparison["agreement_rule_based"]),
        "agreement_with_hmm_walk_forward": _rate(comparison["agreement_hmm"]),
        "risk_off_confirmation_rule_based": float(risk_off.loc[rule_stress & covered].mean())
        if (rule_stress & covered).any()
        else np.nan,
        "risk_off_confirmation_hmm": float(risk_off.loc[hmm_stress & covered].mean())
        if (hmm_stress & covered).any()
        else np.nan,
        "pre_stress_warning_count": _pre_stress_count(labels, rule, int(lead_window)),
        ...
        "strategy_main": getattr(strategy_recommendation, "main_strategy", None),
        "strategy_overlay": getattr(strategy_recommendation, "overlay_strategy", None),
